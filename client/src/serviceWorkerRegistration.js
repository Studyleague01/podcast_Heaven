...
export function register(config) {
  // Determine if registration should happen
  const shouldRegister = (import.meta.env.PROD || config?.forceDev) && 'serviceWorker' in navigator;
...
function checkValidServiceWorker(swUrl, config) {
  ...
  fetch(swUrl, {
    ...
    .then((response) => {
      ...
      console.log('Service worker response:', response.status, contentType);
      ...
      if (
        ...
        // Try direct registration as a last resort
        console.log('Attempting direct registration of service worker...');
        registerValidSW(swUrl, config);
      } else {
    ...
    .catch((error) => {
      ...
      // Try direct registration when fetch fails (could be CORS or other network issues)
      registerValidSW(swUrl, config);
    });
}
